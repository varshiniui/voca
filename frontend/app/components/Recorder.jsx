...
function friendlyError(err) {
  ...
  if (msg.includes('429') || msg.includes('quota') || msg.includes('RESOURCE_EXHAUSTED'))
    return 'AI quota reached — please wait 30 seconds and try again.'
  if (msg.includes('NotAllowedError'))    return 'Microphone access denied. Allow it in your browser settings.'
  if (msg.trim().startsWith('{'))        return 'Something went wrong on the server. Please try again.'
  return msg
}
    ...
    <>
      <style>{`
        ...
        .r-lang-dd {
          ...
          border-radius: 16px; overflow: hidden; min-width: 185px;
          box-shadow: 0 16px 48px rgba(45,32,22,0.16);
        }
        .r-lang-opt {
          ...
          color: var(--ink2);
          border-bottom: 1px solid var(--border2); transition: all .12s;
        }
        .r-lang-opt:last-child { border-bottom: none; }
        .r-lang-opt:hover { background: var(--coral-bg); color: var(--coral); }
        .r-lang-opt.sel { color: var(--coral); }
        ...
        .r-mic-idle {
          ...
          box-shadow: 0 0 0 1.5px rgba(240,118,90,0.25),
                      0 6px 28px rgba(240,118,90,0.18),
                      inset 0 1px 0 rgba(255,255,255,0.8);
        }
        .r-mic-idle:hover {
          transform: scale(1.08) translateY(-3px);
          box-shadow: 0 0 0 2px rgba(240,118,90,0.4),
                      0 14px 44px rgba(240,118,90,0.28),
                      inset 0 1px 0 rgba(255,255,255,0.8);
        }
        .r-mic-rec {
          background: linear-gradient(135deg, #f0765a 0%, #d45a3a 100%);
          box-shadow: 0 0 0 2px rgba(240,118,90,0.5),
                      0 8px 32px rgba(240,118,90,0.4),
                      inset 0 1px 0 rgba(255,255,255,0.25);
          animation: pulse-soft 1.6s ease-in-out infinite;
        }
        .r-mic-rec:active { transform: scale(.95); }
        .r-mic::before {
        ...
        .r-play-btn {
          ...
          display: flex; align-items: center; justify-content: center;
          cursor: pointer; flex-shrink: 0; transition: all .15s; color: white;
          box-shadow: 0 3px 10px rgba(240,118,90,0.3);
        }
        ...
        .r-btn-ghost:hover { background: var(--card); border-color: var(--ink4); color: var(--ink); }

        .r-btn-primary {
          width: 100%; border-radius: 14px; padding: 14px;
          background: var(--coral); border: none; color: white; cursor: pointer;
          font-family: var(--font-body); font-weight: 800; font-size: 14px;
          display: flex; align-items: center; justify-content: center; gap: 8px;
          transition: all .18s;
          box-shadow: 0 4px 18px rgba(240,118,90,0.35);
        }
        ...
        .r-tx {
          ...
          background: var(--bg2); border: 1.5px solid var(--border);
          color: var(--ink); font-size: 13.5px; line-height: 1.75; outline: none;
          font-family: var(--font-body); transition: border-color .15s;
          min-height: 110px;
        }
        .r-tx:focus { border-color: rgba(240,118,90,0.45); background: var(--card); }
        .r-tx::placeholder { color: var(--ink4); }

        /* ── Error ── */
        .r-err {
          width: 100%; padding: 11px 14px; border-radius: 12px;
          background: #fff5f5; border: 1.5px solid rgba(220,38,38,0.2);
      ...
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 20, width: '100%' }}>
        <AnimatePresence mode="wait">
                      ...
                      >
                        {LANGUAGES.map(lang => (
                          <button key={lang.label}
                            className={`r-lang-opt ${language.label === lang.label ? 'sel' : ''}`}
              ...
              {stage === 'recording' ? (
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10 }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: 3, height: 26 }}>
                    {['var(--coral)','var(--amber)','var(--sage)','var(--sky)','var(--coral)','var(--amber)','var(--sage)','var(--sky)','var(--coral)','var(--amber)','var(--sage)','var(--sky)'].map((c, i) => (
                      <motion.div key={i}
                        style={{ width: 3, borderRadius: 2, background: c, opacity: 0.75 }}
                ...
                <div style={{ textAlign: 'center' }}>
                  <p style={{ fontSize: 14, color: 'var(--ink2)', fontWeight: 600 }}>
                    Tap to start recording
                  </p>
                  <p style={{ fontSize: 11.5, color: 'var(--ink4)', marginTop: 5, fontWeight: 600 }}>
          ...
          {stage === 'preview' && (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                ...
                <span style={{ fontSize: 11, color: 'var(--ink3)', padding: '3px 11px',
                  borderRadius: 99, border: '1.5px solid var(--border)', background: 'var(--bg2)', fontWeight: 700 }}>
                  {language.flag} {language.label}
                </span>
              </div>
              ...
              <div className="r-player">
                ...
                <span style={{ fontSize: 11, color: 'var(--ink3)', fontWeight: 700, fontVariantNumeric: 'tabular-nums' }}>
                  {fmt(recTime)}
                </span>
              </div>
              ...
              <button onClick={transcribeAudio} style={{
                ...
                color: 'var(--ink4)', fontFamily: 'var(--font-body)', fontWeight: 700,
                textDecoration: 'underline', textDecorationStyle: 'dotted',
                textDecorationColor: 'var(--ink4)', textUnderlineOffset: 3,
          ...
          {stage === 'editing' && (
            ...
            >
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                <span style={{ fontSize: 10, fontWeight: 800, color: 'var(--ink3)',
                  letterSpacing: '.12em', textTransform: 'uppercase', fontFamily: 'var(--font-body)' }}>
                  Transcript · {language.flag} {language.label}
                </span>
                <span style={{ fontSize: 11.5, color: 'var(--coral)', fontWeight: 700 }}>edit if needed ✏️</span>
              </div>
              <textarea className="r-tx"
